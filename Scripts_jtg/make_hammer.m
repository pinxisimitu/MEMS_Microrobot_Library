...
if ~isfield(h_hammer,'h_relay')
    h_hammer.h_relay.contact_type = 1;
    h_hammer.h_relay.contact_head_type = 1;
    h_hammer.h_relay.springw = 5;
    h_hammer.h_relay.dc_offset = 3;
end




if ~isfield(h_hammer,'vapr_relay')
    h_hammer.vapr_relay = 0;
end

if ~isfield(h_hammer,'recess_ham')
    h_hammer.recess_ham = 0;
end
...
if ~isfield(h_hammer,'h_latch')
        ...
        h_hammer.h_latch.ss.spring_w = 4;
end

if ~isfield(h_hammer.h_latch.ss,'spring_w')
        h_hammer.h_latch.ss.spring_w = 4;
end
...
h_ham.recess_ham = h_hammer.recess_ham;
...
h_latch.ss.spring_w = h_hammer.h_latch.ss.spring_w;
...
if h_hammer.stages >0
    for k = 1:h_hammer.stages
        ...
        h_latch.vapr_relay = h_hammer.vapr_relay;
        h_latch.h_relay = h_hammer.h_relay;
        
        ev = sprintf('[g_latch_%d latch_ref_pts]= latch2(h_latch);',k);
        eval(ev);
...
elseif (h_hammer.chiplet == 2) %Standard not rotated chiplets
    ...
    fill_y_bottom_lever_arm = layout_bottom - 25 - h_hammer.recess_ham;
    
    
    if h_hammer.compact_latch
        p1 = [central_x(1) - chiplet_w/2 - trench_w/2 central_x(2)+trench_w/2] - [trench_w/2 0] + [0 h_hammer.recess_ham];
        p2 = [central_x(1) + chiplet_w/2 + trench_w/2 central_x(2)+trench_w/2] + [0 h_hammer.recess_ham];
        p3 = [central_x(1) + chiplet_w/2 + trench_w/2 fill_y_bottom_lever_arm - trench_w/2] + [0 h_hammer.recess_ham];
        p4 = [central_x(1) - chiplet_w/2 - trench_w/2 fill_y_bottom_lever_arm - trench_w/2] + [0 h_hammer.recess_ham];
        p5 = p1 + [trench_w/2 0];
    else
        p1 = [central_x(1) - chiplet_w/2 - trench_w/2 central_x(2)+trench_w/2] - [trench_w/2 0] + [0 h_hammer.recess_ham];
        p2 = [layout_right + 150 central_x(2)+trench_w/2] + [0 h_hammer.recess_ham];
        p3 = [layout_right + 150 fill_y_bottom_lever_arm - trench_w/2] + [0 h_hammer.recess_ham];
        p4 = [central_x(1) - chiplet_w/2 - trench_w/2 fill_y_bottom_lever_arm - trench_w/2] + [0 h_hammer.recess_ham];
        p5 = p1 + [trench_w/2 0]+ [0 h_hammer.recess_ham];
    end
    ...
    p1 = [central_x(1) - chiplet_w/2 - trench_w/2 central_x(2)+trench_w/2]+ [0 h_hammer.recess_ham];
    p2 = [central_x(1) + chiplet_w/2 + trench_w/2 central_x(2)+trench_w/2]+ [0 h_hammer.recess_ham];
    p3 = [central_x(1) + chiplet_w/2 + trench_w/2 fill_y_bottom_lever_arm - trench_w/2]+ [0 h_hammer.recess_ham];
    p4 = [central_x(1) - chiplet_w/2 - trench_w/2 fill_y_bottom_lever_arm - trench_w/2]+ [0 h_hammer.recess_ham];
    p5 = p1;
    ...
    if h_hammer.zif==1 % Will not make pegs for zif==3 (jumper)
        peg_w = 350;
        peg_l = 300;
        trap_b2 = 100;  %Base 2 (top) of trapezoid
        trap_h = 150;   %Height or trapezoid 
        ...
        %Trapezoid ontop of left peg
        pp0 = [h_rect.x h_rect.y+peg_l];
        pp1 = pp0 + [(peg_w-trap_b2)/2 trap_h];
        pp2 = pp1 + [trap_b2 0];
        pp3 = pp0 + [peg_w 0];
        
        % Define points in the path [x,y]
        h_shape.pts = [pp0;pp1;pp2;pp3];       % Make sure each row is one point [x,y]
        h_shape.layer = 6;                     % Set layer of shape
        left_top_trap = m_shape(h_shape);             % Function to create the shape GDS structure
        h_shape.layer = 4;                     % Set layer of shape
        left_top_trap_4 = m_shape(h_shape);
        ...
        h_label.layer = 2;
        h_label.height = 40;
        h_label.text = h_hammer.label;
        h_label.p0 = [h_rect.x h_rect.y-60];
        rect_lab = add_label(h_label);  % Add label underneath rectangle
        
        
        
        %Add trench for added peg+trapezoid
        h_rect.x = p1(1) + trench_w/2 - trench_w;
        h_rect.y = p1(2) - trench_w/2;
        h_rect.w = peg_w + 2*trench_w;
        h_rect.l = peg_l + trap_h + trench_w;
        h_rect.layer = 5;
        left_peg_trench = rect(h_rect);
        h_rect.layer = 8;
        left_peg_trench_dummy = rect(h_rect);
        
        %Trapezoid ontop of left peg
        pp0 = [p1(1) + trench_w/2 - trench_w p1(2) - trench_w/2+peg_l];
        pp1 = pp0 + [(2*trench_w+peg_w-trap_b2)/2 trap_h+trench_w];
        pp2 = pp1 + [trap_b2 0];
        pp3 = pp0 + [peg_w+2*trench_w 0];
        
        % Define points in the path [x,y]
        h_shape.pts = [pp0;pp1;pp2;pp3];       % Make sure each row is one point [x,y]
        h_shape.layer = 5;                     % Set layer of shape
        %left_top_trap_trench = m_shape(h_shape);             % Function to create the shape GDS structure
        ...
        h_shape.pts = [pp0;pp1;pp2;pp3];       % Make sure each row is one point [x,y]
        h_shape.layer = 6;                     % Set layer of shape
        right_top_trap = m_shape(h_shape);             % Function to create the shape GDS structure
        h_shape.layer = 4;                     % Set layer of shape
        right_top_trap_4 = m_shape(h_shape);
        
        
        %Add trench for added peg+trapezoid
        h_rect.x = h_rect.x - trench_w;
        h_rect.y = h_rect.y;
        h_rect.w = peg_w + 2*trench_w;
        h_rect.l = peg_l + trap_h + trench_w;
        h_rect.layer = 5;
        right_peg_trench = rect(h_rect);
        h_rect.layer = 8;
        right_peg_trench_dummy = rect(h_rect);
...
%% Add label if it's a vapr relay

if h_hammer.vapr_relay==1
    % Define text label properties
    h_label.layer = 2;
    h_label.height = 40;
    h_label.text = ['reso_w = ' num2str(h_hammer.h_relay.springw)];
    h_label.p0 = h_hammer.p0 + [300 -300];
    relay_label_1 = add_label(h_label);  % Add label underneath rectangle
end
