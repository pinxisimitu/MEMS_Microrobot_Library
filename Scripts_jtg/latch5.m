function [out pts_out] = latch5(h_latch)
...
% Add offset to contact angle to get rid of fillets and attach serptentine
% springs directly to rotor
ang_offset = 10*pi/180;
contact_angle = contact_angle + h_latch.orientation*ang_offset;

contact_w = h_latch.ss_contact_w;          % Width of contact rod affixing serpentine to rotor/extension
contact_w2 = .1;           % This is to change rotary serpentine spring attachment point to rotor itself
s_spring_width = 3.5;     % Actual width of the meanders of the serpentine spring
...
if h_latch.closed==0
    % Creates contact rod at the rotor 
...
h_rect.rp = 1;
h_rect.l = contact_l+70;                % Add extra length to rotate rotary serpentine springs 
...
% Test for serpentine spring
%c1p = c1p_long;

cont2 = midpt(c1p,c2p,1-closeness); %Left end of serpentine
cont1 = midpt(c1p,c2p,0);   %Right ened of serpentine
...
if h_latch.ss_meander_spline ~= 1       %Will run when ss_meander_spline == 0 or 2
...
h_path.w = h_latch.ss.dpp*1.5;                           % Width of path
