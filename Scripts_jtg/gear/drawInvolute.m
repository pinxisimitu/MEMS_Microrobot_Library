function [bigX bigY EH_pts] = drawInvolute(P,N,PA,x,y,phi,angles)
% handle = drawInvolute(P,N,PA,x,y,phi)
% 
...
% angles = [theta_low theta_high] and dictates what angles of the gear will
% have teeth on it
...
for i=1:rows
    ...
    ypts = [ypts curr_y - w_step/2 + cumsum(w_step*ones(1,num_eh))];
end
...
%Find range based on angles
rlow = round(N*angles(1)/360);
rhigh = round(N*angles(2)/360);
%Calculate how many teeth we need
nn = pi/180*(angles(2) - angles(1))/(1.3*angThick);



for i = 1:nn
        bigT = [bigT, Ttooth + 2*(i-1)*theta_mod + angles(1)*pi/180];
        bigR = [bigR, Rtooth];
        
        bigT_eh = [bigT_eh, EH_pts_T + 2*(i-1)*theta_mod + angles(1)*pi/180];
...
bigX = [bigR bigR(1)] .* cos([bigT bigT(1)] - angThick/2 + phi) + x;
bigY = [bigR bigR(1)] .* sin([bigT bigT(1)] - angThick/2 + phi) + y;

bigX_eh = [bigR_eh] .* cos([bigT_eh] - angThick/2 + phi) + x;
bigY_eh = [bigR_eh] .* sin([bigT_eh] - angThick/2 + phi) + y;
