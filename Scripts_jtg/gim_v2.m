...
h_rect.x = h_gim.p0(1) - 50;
h_rect.y = h_gim.p0(2) - 750;
h_rect.w = h_gim.w + 100;
h_rect.l = 1500;
h_rect.layer = 8;
h_rect.rounded = 0;
...
[g_latch latch_ref_pts]= latch4(h_latch);
...
[g_latch2 latch_ref_pts]= latch4(h_latch);
...
if h_gim.manual == 0        %Should we draw a motor?
    ...
    gap = 20;
    offset = 64;
    init = [h_rect.x+h_rect.w h_rect.y - 300];
    ...
    for i=1:num_pads
        ...
        h_rect.x = offset + init(1) + (i-1)*(pad_w);
        h_rect.y = init(2);
        h_rect.w = pad_w;
        h_rect.l = pad_w;
        h_rect.layer = 8;
    ...
    % Top right HV line (2nd from left)
    p1 = rps(2,:);
    ...
    % Bottom left HV line (fartherst to the right)
    p1 = rps(4,:);
    ...
    % Bottom left HV line (2nd from the right)
    p1 = rps(3,:);
